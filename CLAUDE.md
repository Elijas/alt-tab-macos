...
```bash
pkill -f "AltTab Fork" 2>/dev/null; sleep 1; open ~/Library/Developer/Xcode/DerivedData/alt-tab-macos/Build/Products/Debug/AltTab\ Fork.app
```

# Building

```bash
xcodebuild -workspace alt-tab-macos.xcworkspace -scheme Debug -configuration Debug -destination 'platform=macOS,arch=arm64' CODE_SIGN_IDENTITY="-" -derivedDataPath ~/Library/Developer/Xcode/DerivedData/alt-tab-macos build
```
