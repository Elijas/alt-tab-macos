...
private final class MenubarMenuDelegate: NSObject, NSMenuDelegate {
    ...
    func menuWillOpen(_ menu: NSMenu) {
        ...
        PanelMenu.shared.refreshCheckmarks(in: menu)
    }
}

class PanelMenu: NSObject {
    ...
    func refreshCheckmarks(in menu: NSMenu) {
        for item in menu.items {
            guard let ref = item.representedObject as? ToggleRef else { continue }
    ...
    @objc private func toggleItem(_ sender: NSMenuItem) {
        ...
        Preferences.set(ref.key, newValue ? "true" : "false")
        ref.apply()
