...
class SidePanelManager {
    ...
    private func refreshPanelsNow() {
        ...
        // debug: dump full window state to file
        var lines = ["═══ SIDE PANEL REFRESH ═══"]
        for w in Windows.list {
            let wid = w.cgWindowId.map { String($0) } ?? "nil"
            let app = w.application.localizedName ?? "?"
            let title = w.title ?? ""
            let sid = (w.screenId as String?) ?? "nil"
            let spaces = w.spaceIds.map { String($0) }
            let bl = self.isBlacklisted(w) ? " BL" : ""
            lines.append("  wid=\(wid) lfo=\(w.lastFocusOrder) scr=\(sid) sp=\(spaces) \(app) – \(title)\(bl)")
        }
        lines.append("  visibleSpaces=\(Spaces.visibleSpaces)")
        lines.append("═══════════════════════════")
        try? lines.joined(separator: "\n").appending("\n").write(toFile: "/tmp/panel-debug.log", atomically: true, encoding: .utf8)
        ...
        for (_, panel) in panels {
            ...
            var plines = ["  ── panel screen=\(screenUuid) spaces=\(sortedSpaces) ──"]
            for (si, spaceId) in sortedSpaces.enumerated() {
                let spaceIndex = Spaces.idsAndIndexes.first { $0.0 == spaceId }?.1 ?? -1
                // all windows CGS knows about on this space
                let allOnSpace = Spaces.windowsInSpaces([spaceId])
                _ = allOnSpace.count // used below
                // only non-invisible (non-tabbed) windows on this space
                let visibleOnSpace = Set(Spaces.windowsInSpaces([spaceId], false))

                if si > 0 { plines.append("    ──────") }
                plines.append("    space \(spaceIndex) (id=\(spaceId)): cgs_all=\(allOnSpace.count) cgs_visible=\(visibleOnSpace.count)")
                ...
                for wid in allOnSpace {
                    ...
                    if let window = windowByCgId[wid] {
                        let reasons = [
                            seen.contains(wid) ? "seen" : nil,
                            window.isWindowlessApp ? "windowless" : nil,
                            window.isMinimized ? "minimized" : nil,
                            window.isHidden ? "hidden" : nil,
                            !isVisible ? "tabbed" : nil,
                            self.isBlacklisted(window) ? "blacklisted" : nil,
                            panelWindowNumbers.contains(Int(wid)) ? "panel" : nil,
                        ].compactMap { $0 }
                        let app = window.application.localizedName ?? "?"
                        let title = window.title ?? ""
                        if reasons.isEmpty {
                            plines.append("      ✓ wid=\(wid) \(app) – \(title)")
                            group.append(window)
                        } else {
                            plines.append("      ✗ wid=\(wid) \(app) – \(title) [\(reasons.joined(separator: ","))]")
                        }
                    } else {
                        plines.append("      ? wid=\(wid) not_in_Windows.list vis=\(isVisible)")
                    }
                }
            ...
            if let existing = try? String(contentsOfFile: "/tmp/panel-debug.log", encoding: .utf8) {
                try? (existing + plines.joined(separator: "\n") + "\n").write(toFile: "/tmp/panel-debug.log", atomically: true, encoding: .utf8)
            }
