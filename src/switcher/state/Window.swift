...
class Window {
    ...
    private struct FocusSnapshot {
        ...
        let originSpaceId: CGSSpaceID
        let originFrontPid: pid_t?
        ...
        if let altTabWindow = altTabWindow() {
            ...
            BackgroundWork.accessibilityCommandsQueue.addOperation { [weak self] in
                ...
                if let tab = snapshot.tab {
                    ...
                    makeKeyWindow(&psn, snapshot.targetCgWindowId)
                ...
                if let originFrontPid = snapshot.originFrontPid, originFrontPid != snapshot.pid {
                    var originPsn = ProcessSerialNumber()
                    GetProcessForPID(originFrontPid, &originPsn)
                    SLSSpaceSetFrontPSN(CGS_CONNECTION, snapshot.originSpaceId, originPsn)
                }
    ...
    private static func focusTabChild(_ snapshot: FocusSnapshot, _ tab: TabFocusSnapshot, _ psn: inout ProcessSerialNumber) {
        ...
        let needsSpaceSwitch = !visibleSpaceIds.contains(where: { parentSpaceIds.contains($0) })
        _SLPSSetFrontProcessWithOptions(&psn, tab.parentCgWindowId, SLPSMode.userGenerated.rawValue)
        if needsSpaceSwitch {
            makeKeyWindow(&psn, tab.parentCgWindowId)
    ...
    private static func waitForVisibleSpace(_ parentSpaceIds: Set<CGSSpaceID>) {
        for i in 0..<40 {
            ...
            if visibleSpaces.contains(where: { parentSpaceIds.contains($0) }) { break }
        }
    }
